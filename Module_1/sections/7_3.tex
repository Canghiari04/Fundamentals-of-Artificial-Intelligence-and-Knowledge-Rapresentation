...
\begin{example}
    ...
    \begin{itemize}
        ...
        \item $1^{st}$ step: define the domain theory, formal descriptions about all the executable actions. \vspace{3.5pt}
            ...
            \begin{center}
                $on(X,Y,S), clear(X,S), clear(Z,S), \mathit{diff}(X,Z) \rightarrow clear(Y, do(move(X,Y,Z), S)), on(x,z,do(move(X,Y,Z), S))$
            \end{center} \vspace{3.5pt}
    ...
    \begin{itemize}
        ...
        \item $1^{st}$ step: as before, define the initial state in First Order Logic. \vspace{3.5pt}
            \begin{itemize}[nosep]
                ...
                \item $holds(clear(c),S_0)$ 
                \item $holds(clear(a),S_0)$ 
                \item $holds(clear(b),S_0)$
            \end{itemize} \vspace{3.5pt} 
        ...
        \begin{center}
            $pact(move(X,Y,Z), S)$ :- \\ $holds(on(X,Y), S) \land holds(clear(X), S) \land holds(clear(Z), S)$ 
        \end{center}
        \item $4^{th}$ step: define the frame conditions, describing the fluents that remain unchanged after the action's execution.
        \begin{center}
            $holds(V, do(move(X,Y,Z), S))$ :- \\ $holds(V, S) \land \textit{diff}(V, on(X,Y)) \land \textit{diff}(V, clear(X)) \land \textit{diff}(V, clear(Z))$ 
        \end{center}
