...
\begin{example}
    ...
    \begin{itemize}[nosep]
        \renewcommand{\labelitemi}{-}
        \item Performing the action $pickup(b)$ negates the $handempty$ precondition necessary for the predicate $pickup(a)$. This problem can be easily solved by enforcing a \textbf{promotion} ordering constraint. The action $pickup(a)$ must be executed after the predicate $stack(b,c)$, in other words $stack(b,c) < pickup(a)$.
    ...
    \begin{center}
        ...
        \includegraphics[width=0.75\textwidth]{img/img26.png}
    \end{center} \vspace{3.5pt}
\end{example}
Unfortunately \textbf{non-linear planners} can generate very inefficient plans, even tough they are correct. In domains of increasing complexity it's hard to use non-linear planners,
even they are complete. Usually the common way to resolve difficult domains is done by \textbf{graph planning}.