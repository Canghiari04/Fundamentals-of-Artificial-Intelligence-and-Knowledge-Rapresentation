...
\begin{example}
    ...
    \begin{center}
        ...
        \end{tabular}
    \end{center} \vspace{3.5pt}
    define a plan that reach the goal state \vspace{7pt}
    \begin{center}
        \begin{tabular}{l}
            \textbf{Goal state} \\
            $on(b,c) \land on(c,a)$
    ...
    Here we are attempting to solve the \textbf{Sussman anomaly} by POP. As usual, we have to define the initial empty plan, containing two fake actions: the start action and
    ...
    \begin{itemize}[nosep]
        ...
        \item $3^{rd}$ step: 
            \begin{itemize}[nosep]
                ...
                \item Select an action that has the preconditions $holding(b)$ as postcondition: $pickup(b)$ or $unstack(b,Y)$.
        ...
        \item $4^{th}$ step:
            \begin{itemize}[nosep]
                \renewcommand{\labelitemii}{-}
                \item Exactly the same procedure described in the previous step happens for $stack(a,c)$ predicate.
            \end{itemize} \vspace{3.5pt}
